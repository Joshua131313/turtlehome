...
export const getTimeAgo = (date, isActiveAgo) => {
  ...
  if (isActiveAgo) {
    ...
    else return "Last active: " + convertClassicDate(date);
  } else {
    if (seconds < 1) return "Just now";
    else if (seconds < 60) return seconds + ` second${addS(seconds)} ago`;
    else if (seconds < 3600)
      return (
        Math.floor(seconds / 60) +
        ` minute${addS(Math.floor(seconds / 60))} ago`
      );
    else if (seconds < 86400)
      return (
        Math.floor(seconds / 3600) +
        ` hour${addS(Math.floor(seconds / 3600))} ago`
      );
    else if (seconds < 604800)
      //if less than 3 days
      return (
        Math.floor(seconds / 86400) +
        ` day${addS(Math.floor(seconds / 86400))} ago`
      );
    else return convertClassicDate(date);
  }
};
